...
export function SongDashboard({
	...
	const { data, isLoading } = useSongReply({
		guildId,
		auth,
		visitor,
		bearer,
					...
					<div className="flex flex-col gap-4">
						...
						<div className="flex flex-col gap-2 justify-center items-center">
							...
							{data.skipToTimestamp && (
								<Button
									onClick={() => {
										handleClick(SongEditType.SkipSegment);
									}}
									className="w-full"
								>
						...
						{data.song ? (
							<div className="flex flex-col gap-2 w-full">
								...
								<div className="flex flex-col">
									<div className="w-full">
										<PlaybackControl
											now={Math.max(time ?? 0, 0)}
											totalTime={data.song.duration}
											enabled={data.canSeek}
											onRelease={async (releaseTime) => {
												if (
													!(await editAction(
														SongEditType.SetTime,
														authData,
														Math.round(releaseTime),
													))
												) {
													return toast(
														"Failed to seek",
